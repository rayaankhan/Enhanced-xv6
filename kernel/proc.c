...
void scheduler(void)
{
  ...
  while(1)
  {
    ...
    int curtotal = 0, total = 0, winticket;
    // time_t t;
    for (p = proc; p < &proc[NPROC]; p++)
    {
      // acquire(&p->lock);
      ...
      {
        // release(&p->lock);
        total = total + p->ticket;
      }
    }
    // printf("%d\n",total);
    // printf("pohoch gyi!\n");
    winticket = myRandInRange(0, total);
    // printf("%d\n",winticket);
    for (p = proc; p < &proc[NPROC]; p++)
    {
      acquire(&p->lock);
      ...
      {
        // printf("sahi chal rha hai!\n");
        curtotal = curtotal + p->ticket;
        // printf("else sahi chal rha hai!\n");
        // release(&p->lock);
        // continue;
      }
      if (curtotal > winticket)
      {
        // printf("abhi bhi sahi chal rha hai!\n");
        int tstart, tend;
        c->proc = p;
        p->state = RUNNING;
        tstart = ticks;
  ...
  while(1)
  {
    ...
    for (p = proc; p < &proc[NPROC]; p++)
    {
      ...
      // if the process has either waited or has ran or both for sometime
      ...
      if (p->state == RUNNABLE)
      {
        ...
        else if (highest_priority_process->createTime > p->createTime)
        {
          ...
          dynamicPriority = process_dp; // set the dynamic priority
          highest_priority_process = p; // set the process with the highest priority
          continue;
        }
      }
  ...
  for (;;)
  {
    ...
    for (p = proc; p < &proc[NPROC]; p++)
    {
      ...
      if (p->state == RUNNABLE)
      {
        ...
        if (req->createTime < p->createTime && i != 0)
        {
          release(&req->lock);
          req = p;
          continue;
